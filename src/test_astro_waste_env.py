...
def main():
	...
	env = ToxicWasteEnvV2(field_size, layout, n_players, n_objects, max_episode_steps, RNG_SEED, data_dir, facing, centered_obs,
	                      slip=has_slip, is_train=False, use_render=use_render, pick_all=False, problem_type=problem_type)
	...
	for i in range(N_CYCLES):
		...
		while not done:
			...
			for idx in range(n_players):
				...
				logger.info('Player %s at (%d, %d) with orientation (%d, %d) and chose action %s' % (env.players[idx].name, *env.players[idx].position,
				 																							*env.players[idx].orientation, Actions(action).name))
