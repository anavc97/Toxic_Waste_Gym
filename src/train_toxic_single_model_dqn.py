...
def model_execution(human_dqn_model: DQNetwork, robot_dqn_model: DQNetwork, eps: float, greedy_actions: bool, n_agents: int, obs: np.ndarray,
					rng_gen: np.random.Generator, waste_env: Union[ToxicWasteEnvV1, ToxicWasteEnvV2], episode_q_vals: List) -> List[int]:
	...
	if rng_gen.random() < eps:
		...
		actions = []
		for a_idx in range(n_agents):
			a_id = waste_env.players[a_idx].name
			model_obs = get_model_obs(obs[a_idx])
			if a_id == 'robot':
				q_values = human_dqn_model.q_network.apply(human_dqn_model.online_state.params, model_obs[0], model_obs[1])[0]
			else:
				q_values = robot_dqn_model.q_network.apply(robot_dqn_model.online_state.params, model_obs[0], model_obs[1])[0]
...
def train_astro_model_v2(waste_env: ToxicWasteEnvV2, human_model: SingleAgentDQN, astro_model: SingleAgentDQN, heuristic_models: List[GreedyAgent], num_iterations: int,
	...
	human_idx = agents_ids.index('human')
	if interactive:
	...
	obs, *_ = waste_env.reset()
	astro_dqn_model = astro_model.agent_dqn
	robot_obs = get_model_obs(obs[robot_idx])
	human_obs = get_model_obs(obs[human_idx])
	astro_model.agent_dqn.init_network_states(rng_seed, (robot_obs[0], robot_obs[1]), optim_learn_rate, curriculum_model)
	human_model.agent_dqn.init_network_states(rng_seed, (human_obs[0], human_obs[1]), optim_learn_rate, curriculum_model)
	if restart:
	...
	for it in range(start_it, num_iterations):
		...
		while not done:
			# interact with environment
			if anneal:
				actions = heuristic_execution(waste_env, heuristic_models)
			else:
				if eps_type == 'epoch':
					eps = DQNetwork.eps_update(EPS_TYPE[eps_type], initial_eps, final_eps, exploration_decay, epoch, max_timesteps)
				else:
				...
				actions = model_execution(human_model.agent_dqn, astro_dqn_model, eps, greedy_actions, n_agents, obs, decision_rng_gen, waste_env, episode_q_vals)
			...
			if astro_dqn_model.use_summary:
				astro_dqn_model.summary_writer.add_scalar("charts/reward", step_reward, epoch)
			...
			# store new samples
			human_model.replay_buffer.add(obs[human_idx], next_obs[human_idx], np.array(actions[human_idx]), rewards[human_idx], finished[human_idx], [infos])
			astro_model.replay_buffer.add(obs[robot_idx], next_obs[robot_idx], np.array(actions[robot_idx]), rewards[robot_idx], finished[robot_idx], [infos])
			obs = next_obs
			
			# update Q-network and target network
			astro_model.update_dqn_models(batch_size, epoch, start_time, target_freq, tau, summary_frequency, train_freq, warmup)
			human_model.update_dqn_models(batch_size, epoch, start_time, target_freq, tau, summary_frequency, train_freq, warmup)
			...
			if terminated or timeout:
				...
				if astro_dqn_model.use_summary:
					sum_qs = sum(episode_q_vals)
					n_qs = len(episode_q_vals)
					astro_dqn_model.summary_writer.add_scalar("charts/episode_q_vals", sum_qs, epoch)
					astro_dqn_model.summary_writer.add_scalar("charts/mean_episode_q_vals", sum_qs / n_qs if n_qs > 0 else 0, epoch + start_record_epoch)
					astro_dqn_model.summary_writer.add_scalar("charts/episode_return", episode_rewards, it + start_record_it)
					astro_dqn_model.summary_writer.add_scalar("charts/avg_episode_return", np.mean(avg_episode_reward), it + start_record_it)
					astro_dqn_model.summary_writer.add_scalar("charts/episodic_length", episode_len, it + start_record_it)
					astro_dqn_model.summary_writer.add_scalar("charts/avg_episode_len", np.mean(avg_episode_len), it + start_record_it)
					astro_dqn_model.summary_writer.add_scalar("charts/epsilon", eps, it + start_record_it)
					astro_dqn_model.summary_writer.add_scalar("charts/anneal_temp", temp, it + start_record_it)
					astro_dqn_model.summary_writer.add_scalar("charts/iteration", it, it + start_record_it)
				obs, *_ = waste_env.reset()
				if waste_env.use_render:
					waste_env.render()
				else:
